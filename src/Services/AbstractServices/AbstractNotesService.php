...
/**
 * This class is responsible from managing the data for Notes
 *
 * Class NotesService.
 *
 * @package NextDeveloper\CRM\Database\Models
 */
class AbstractNotesService
{
    public static function get(NotesQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new NotesQueryFilter(new Request());
        }
     ...
     * @return mixed
     */
    public static function getByRef($ref) : ?Notes
    {
        return Notes::findByRef($ref);
    }
    ...
    {
        $object = Notes::where('uuid', $objectId)->first();

        $action = '\\NextDeveloper\\CRM\\Actions\\Notes\\' . Str::studly($action);
     ...
     * @return Notes|null
     */
    public static function getById($id) : ?Notes
    {
    ...
    {
        try {
            $obj = Notes::where('uuid', $uuid)->first();
     ...
     * @return Notes
     */
    public static function updateRaw(array $data) : ?Notes
    {
    ...
    {
        $model = Notes::where('uuid', $id)->first();
